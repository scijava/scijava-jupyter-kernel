...
public class HTMLTableBuilder {
	...
	public static String getTableStyle(final boolean displayRowLabel) {
		String style = "<style>"
				...
				+ "table.converted td {text-align: left;}" + "table.converted tr:nth-child(even) {background: #F3F3F3;}"
				+ "table.converted tr:nth-child(odd) {background: #FFFFFF;}"
				+ "table.converted tbody tr:hover {background: #BDF4B5;}";

		if (!displayRowLabel)
			style += "table.converted td.rowLabel, table.converted th.rowLabel {display: none;}";
		style += "</style>";
	...
	public static String appendHeadings(final String data, final boolean end) {
	...
	public static String appendData(final String data, final boolean start, final boolean end) {
		String html = "";
		if (start)
			html += "<tr>";
		html += "<td>" + data + "</td>";
